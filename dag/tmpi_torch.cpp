...
template <typename T>
std::tuple<torch::Tensor, torch::Tensor>  h_fast_geo(torch::Tensor B, T eps) {
    auto d = B.sizes()[0];
    auto _g = torch::clone(B);
    auto _B = torch::clone(B);
    auto _grad = torch::matrix_power(B, 0);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("h_fast_geo_double", &h_fast_geo<double>, "H_fast_geo native run");
  m.def("h_fast_geo_float", &h_fast_geo<float>, "H_fast_geo native run");
}
